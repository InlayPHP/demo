...
export default function ResourceIndex({
    ...
    const dialogForm = useMemo<FormResource | null>(() => {
        if (!createForm || !modal || modal.mode === 'view' || modal.mode === 'import') {
            return null;
        }
    ...
    const submitImport = () => {
        ...
        setProcessing(true);
        router.post(importEndpoint, { file }, {
            forceFormData: true,
            preserveScroll: true,
            onFinish: () => setProcessing(false),
        });
    ...
    const handleAction = async (
        ...
        if (action.lifecycle && action.url) {
            const data = action.bulk
                ? selection
                    ? { selection }
                    : { records: rows.map((candidate) => candidate[table.primaryKey]) }
                : action.data ?? {};
        ...
        if (action.url) {
            const url = interpolateActionUrl(action.url, row);
            if (url) {
                router.visit(url, { method: action.method as never, data: (action.data ?? {}) as never });
            }
        }
    };

    const modalHeading = modal?.mode === 'edit'
        ? `Edit ${resource.label}`
        : modal?.mode === 'view'
            ? resource.label
            : modal?.mode === 'create'
    ...
    return (
        <InlayPanelLayout>
            ...
            {manageMode && (modal?.mode === 'create' || modal?.mode === 'edit')
                ? (
                        <RelationDialog
                            errors={errors}
...
function ViewRecordModal({
    ...
    const rows: Array<{ label: string; value: string | null; placeholder?: string }> = [
        { label: 'Name', value: String(record.name ?? '') },
        { label: 'Slug', value: String(record.slug ?? '') },
        { label: 'Description', value: record.description ? String(record.description) : null, placeholder: 'No description' },
        { label: 'Visibility', value: record.is_visible ? 'Visible' : 'Hidden' },
        { label: 'Last modified at', value: record.updated_at ? String(record.updated_at) : null },
    ];
                ...
                <dl className="grid gap-4 pt-5 sm:grid-cols-[minmax(0,9rem)_1fr] sm:gap-x-4 sm:gap-y-3">
                    {rows.map((row) => (
                        <div className="contents" key={row.label}>
                            <dt className="text-sm font-medium text-(--inlay-muted)">{row.label}</dt>
                            <dd className="text-sm text-(--inlay-text)">
                                {row.value ?? <span className="text-(--inlay-muted)">{row.placeholder}</span>}
                            </dd>
                        </div>
                    ))}
                </dl>
            ...
            >
                <header className="flex items-start justify-between gap-4 border-b border-(--inlay-border) pb-4">
                    <div>
                        <h3 className="text-xl font-semibold tracking-tight">{heading}</h3>
                        <p className="text-base/7 text-(--inlay-muted) sm:text-sm/6">
                            Upload a CSV with columns name, slug, description, is_visible, seo_title,
                            and seo_description.
                        </p>
                    </div>
                    <button
